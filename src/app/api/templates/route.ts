...
function render(title: string, recipient = "[Cơ quan/tổ chức nhận]", name = "[Họ tên]", address = "[Địa chỉ]", body = "") {
  return `${title}\n\nKính gửi: ${recipient || "[Cơ quan/tổ chức nhận]"}\n\nTôi là: ${name || "[Họ tên]"}\nĐịa chỉ: ${address || "[Địa chỉ]"}\n\n${body}\n\nTài liệu kèm theo: [Liệt kê chứng cứ]\n\nNgười làm đơn\n(Ký và ghi rõ họ tên)\n${name || "[Họ tên]"}`;
}
